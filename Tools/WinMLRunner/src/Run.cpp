    ...
    try
    {
        if (deviceCreationLocation == DeviceCreationLocation::UserD3DDevice && deviceType != DeviceType::CPU)
        {
            ...
            if (args.IsPerformanceCapture())
            {
             ...
             THROW_IF_FAILED(spFactory->CreateAdapterList(dxGUIDs, ARRAYSIZE(dxGUIDs), IID_PPV_ARGS(spAdapterList.put())));
             ...
             {
                 THROW_IF_FAILED(spAdapterList->GetAdapter(i, IID_PPV_ARGS(currAdapter.put())));
